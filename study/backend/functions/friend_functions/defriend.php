...
function defriend($recieve_user,$active_user){
    global $conn;
    $stmt = $conn->prepare("DELETE FROM friend_tbl WHERE user_action = :user2 AND user_recieve = :user1 OR user_action = :user3 AND user_recieve = :user4 ");
    $stmt->bindParam(':user1', $active_user);
    $stmt->bindParam(':user2', $recieve_user);
    $stmt->bindParam(':user3', $active_user);
    $stmt->bindParam(':user4', $recieve_user);
    $stmt->execute();
}
